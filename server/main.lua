...
--[[ 				secondInv.label = other.label.." "..para ]]
				secondInv.label = '<i class="fas fa-shopping-basket"></i>'.." "..other.label.." - Nakit: "..'<span style="color:#479854;">'..para.."$</span>".." "..''
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Böyle bir eşya bulunamadı.", "error")
		end
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = QBCore.Shared.SplitStr(fromInventory, "-")[2]
		local fromItemData = Trunks[plate].items[fromSlot]
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
			TriggerClientEvent('QBCore:Notify', src, "Gerekli eşyalara sahip değilsin..", "error")
		end
	elseif fromInventory == "attachment_crafting" then
		local itemData = Config.AttachmentCrafting["items"][fromSlot]
		if hasCraftItems(src, itemData.costs, fromAmount) then
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot, label)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Bu eşyadan yeteri kadarına sahip değilsin", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Vermek için kullanabileceğin bir eşya yok", "error")
	end
end)
